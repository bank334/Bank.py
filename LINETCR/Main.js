...
var myBott = ['ua3197f4f6dbf0084c535d0c323dd3d7e','ua761fa9918bedbb642a4b144440d1eed','ud0b4dd10c4d54477ec3f9e297d00f2a6','ub0e7d982d8afa8b248a0e86baf5d6507'];//TARO MID LU DISINI SUPAYA BISA PKE COMMAND STAFF

const myBot = ['ua3197f4f6dbf0084c535d0c323dd3d7e','ua761fa9918bedbb642a4b144440d1eed','ud0b4dd10c4d54477ec3f9e297d00f2a6','ub0e7d982d8afa8b248a0e86baf5d6507'];//TARO MID LU DISINI
...
class LINE extends LineAPI {
    ...
    async textMessage(textMessages, seq, param, lockt) {
		...
		if(vx[1] == "msg" && seq.from == vx[0] && waitMsg == "yes"){
            ...
            this._sendMessage(seq,"เสียใจด้วย ควยสั่งไม่ได้");
      }
      ...
      if(txt == 'tab:add:staff' && this.sendStaff == 0 && isAdminOrBot(seq.from)){
         this.sendStaff = 1;
         this._sendMessage(seq,'ส่งติดต่อเพื่อเพิ่มพนักงาน ')
       }
       ...
       if(seq.contentType == 13 && this.sendStaff == 1 && isAdminOrBot(seq.from)) {
          ...
          this._sendMessage(seq,'การเพิ่มพนักงานด้วย Nama :'+'\n'+seq.contentMetadata.displayName);
        }

        if(txt == 'tab:del:staff' && this.sendStaff == 0 && isAdminOrBot(seq.from))
{
           this.sendStaff = 2;
           this._sendMessage(seq,'ส่งข้อมูลติดต่อเพื่อถอดพนักงาน~')
           }
           ...
           if(seq.contentType == 13 && this.sendStaff == 2 && isAdminOrBot(seq.from))
{
              if(!isStaffOrBot(seq.contentMetadata.mid)) {
                 seq.contentType = 0;
                 this.sendStaff = 0;
                 await this._sendMessage(seq,'เค้าไม่ไช้พนักงาน~');
       }
     else
       {
            seq.contentType = 0;
    ...
    this.sendStaff = 0;
    await this._sendMessage(seq,'ถอดพนักงานเรียบร้อยครับนาย~');
    }
}
        ...
        if(txt == 'halo') {
          if(isAdminOrBot(seq.from) || isStaffOrBot(seq.from)) {
        this._sendMessage(seq, 'กราบสวัสดีเพื่อนๆพี่ๆน้องๆทุกคน');
        }
      else
        {
        ...
        if(txt == 'test speed') {
            ...
            await this._sendMessage(seq,'ความเร็ว เร็วกว่าแสง....');
...
let { listMember } = await this.searchGroup(seq.to);
     const mentions = await this.mention(listMember);
        ...
        if(txt == 'creator bot') {
           ...
           seq.contentType=13;
           seq.contentMetadata = { mid: 'ua3197f4f6dbf0084c535d0c323dd3d7e'};
           this._client.sendMessage(1, seq);
        }
            ...
            exec(`curl ipinfo.io/${payload}`,(err, res) => {
                ...
                if(typeof result.error == 'undefined') {
                    ...
                    try {
                        ...
                        this._sendMessage(seq,'ที่ตั่ง');
                    } catch (err) {
                        this._sendMessage(seq,'ไม่พบครับ เพื่อนๆ');
                    }
                } else {
                    this._sendMessage(seq,'ไม่พบสถานที');
